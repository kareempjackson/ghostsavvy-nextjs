...
import { visionProjects } from "./projectData";

// Easing values from brand guidelines
const entryEasing = [0.0, 0.0, 0.2, 1]; // cubic-bezier(0.0, 0.0, 0.2, 1)

export default function UserCentricVision() {
  const containerRef = useRef<HTMLDivElement>(null);
  const isInView = useInView(containerRef, { once: true, amount: 0.2 });

  return (
    <div ref={containerRef} className='py-28 bg-brand-ivory/50 overflow-hidden'>
      <div className='container mx-auto px-6'>
        <motion.div
          ...
          className='max-w-4xl'
        >
          <h2 className='text-5xl md:text-6xl lg:text-7xl text-brand-black font-medium tracking-[-0.02em] leading-[1.1] mb-8'>
            <span className='text-brand-sage'>User-Centric</span> Vision
          </h2>
          <p className='text-xl md:text-2xl text-brand-black/80 leading-normal mb-16 max-w-3xl'>
            We place user needs at the heart of every digital experience,
            creating intuitive interfaces that empower and delight through
            research-driven design and thoughtful implementation.
          </p>
        </motion.div>
          ...
          {visionProjects.map((project, index) => (
            ...
            >
              ...
              >
                ...
                <div className='p-6'>
                  <h3 className='text-xl text-brand-black mb-3 group-hover:text-brand-sage transition-colors'>
                    {project.title}
                  </h3>
